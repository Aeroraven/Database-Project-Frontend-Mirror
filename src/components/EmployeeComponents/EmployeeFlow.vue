                ...
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="6" md="3">
                            <v-text-field :label="$t('employee.item.id')" v-model="submiteId" :placeholder="$t('common.pleaseInput')+$t('employee.item.id')" prepend-icon="el-icon-files"  />
                        </v-col>
                        <v-col cols="12" sm="6" md="3">
                            <v-text-field :label="$t('employee.item.name')" v-model="submitmname" :placeholder="$t('common.pleaseInput')+$t('employee.item.name')" prepend-icon="el-icon-document-copy"  />
                        </v-col>
                        <!-- <v-col cols="12" sm="6" md="3">
                            <v-text-field :label="$t('employee.item.gender')" v-model="submitgender" :placeholder="$t('common.pleaseInput')+$t('employee.item.gender')" prepend-icon="el-icon-time"  />
                        </v-col> -->
...
export default {
    ...
    methods:{
        submitEmployeeInfo(){
            ...
            setTimeout(
                ()=>{
                    createEmployeeInfo().then(response => {
                        ...
                        if(response.data.statcode==1){
                            ...
                            this.errorInfo=this.$t('emploee.NonexistentAniID')
                            return 0;
                        }
                        ...
                        this.submitSuccTip(this.$t('employee.SubmitComplete'))
                    })
                },2000
            )
        },
        ...
        submitPrejudge(){
            ...
            if(this.submitmname==null||this.submitmname==undefined||this.submitmname==0){
                this.submitFailTip(this.$t('员工姓名为空'))
                return 0;
            }
            if(this.submitgender==null||this.submitgender==undefined||this.submitgender==0){
                this.submitFailTip(this.$t('员工性别为空'))
                return 0;
            }
            if(this.submitage==null||this.submitage==undefined||this.submitage==0){
                this.submitFailTip(this.$t('员工年龄为空'))
                return 0;
            }
            if(this.submitwage==null||this.submitwage==undefined||this.submitwage==0){
                this.submitFailTip(this.$t('员工工资为空'))
                return 0;
            }
             if(this.submitposition==null||this.submitposition==undefined||this.submitposition==0){
                this.submitFailTip(this.$t('员工职务为空'))
                return 0;
            }
             if(this.submitpark==null||this.submitpark==undefined||this.submitpark==0){
